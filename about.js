...
// Scrollytelling Setup for About Page
...
const getFramePath = (index) => `./specs-frames/ezgif-frame-${index.toString().padStart(3, '0')}.jpg`;
...
const panels = [
  { id: 'panel-0', start: 0.0, end: 0.18 },
  { id: 'panel-1', start: 0.22, end: 0.48 },
  { id: 'panel-2', start: 0.52, end: 0.78 },
  { id: 'panel-3', start: 0.82, end: 1.0 }
];
...
function preloadImages(onComplete) {
  ...
  const loadingStatuses = [
    "RETRIEVING WALKMAN ARCHIVES...",
    "LOADING D-E100 DISCMAN PATENTS...",
    "COMPILING COGNITIVE AUDIO PATENTS...",
    "STRUCTURING SPATIAL DESIGN PATTERNS...",
    "ALIGNING CHRONICLE GRAPH BLUEPRINTS...",
    "LAUNCHING ABOUT US DIALOGUES..."
  ];
  ...
  for (let i = 1; i <= frameCount; i++) {
    ...
    img.onerror = () => {
      console.warn(`Specs Frame ${i} failed to load. Continuing...`);
      loadedImagesCount++;
      if (loadedImagesCount === frameCount) {
        preloader.classList.add('fade-out');
        initCanvas();
...
function drawFrame(index) {
  ...
  // Use contain to avoid zoom cropping
...
function handleScroll() {
  ...
  // Map scroll fraction to target frame index (1 - 240)
