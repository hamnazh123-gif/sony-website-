...
// Scrollytelling Setup for Side Page
...
const getFramePath = (index) => `./side-frames/ezgif-frame-${index.toString().padStart(3, '0')}.jpg`;
...
function preloadImages(onComplete) {
  ...
  const loadingStatuses = [
    "DIAGNOSING BATTERY STORAGE CAPACITY...",
    "STRESS-TESTING COIL-HEADBAND ANGLE...",
    "MAPPING TOUCH SENSITIVE EAR-CUP MATRIX...",
    "TUNING HIGH-FIDELITY WIRELESS CHIPS...",
    "SIMULATING COGNITIVE SOUNDSTAGE STABILITY...",
    "ALIGNING PROFILE BLUEPRINTS..."
  ];
  ...
  for (let i = 1; i <= frameCount; i++) {
    ...
    img.onerror = () => {
      console.warn(`Side Frame ${i} failed to load. Continuing...`);
      loadedImagesCount++;
      if (loadedImagesCount === frameCount) {
        preloader.classList.add('fade-out');
        initCanvas();
